...
int Bank::login(string name, string password) {
    for (auto &u : users) 
        if (u.getName() == name && u.checkPassword(password)) 
            return u.getId();
    return -1;
}
...
void Bank::saveToFile() {
    ...
    for (auto &u : users) 
        file << "U " << u.getId() << " << u.getName() << endl;
    for (auto &a : accounts) 
        file << "A " << a.getAccountNumber() << " << a.getOwnerId() << " << a.getBalance() << endl;
    file.close();
}
...
void Bank::loadFromFile() {
    ...
    while (file >> type) {
        if (type == "U") { 
            file >> id >> name; 
            users.push_back(User(id, name, "123")); 
        }
        if (type == "A") { 
            file >> acc >> owner >> bal; 
            accounts.push_back(Account(acc, owner)); 
        }
    }
    file.close();
}
